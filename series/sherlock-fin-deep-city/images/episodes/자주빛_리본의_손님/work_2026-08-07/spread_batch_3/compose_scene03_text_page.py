...
def extract_scene03_text(script_path: Path) -> str:
    ...
    if not match:
        raise RuntimeError("Could not locate scene 03 Text block")
    body = match.group(1).strip()
    if body != EXPECTED_TEXT:
        raise RuntimeError("Scene 03 text differs from the approved exact text")
    return body
...
def main() -> None:
    ...
    body = extract_scene03_text(args.script)
